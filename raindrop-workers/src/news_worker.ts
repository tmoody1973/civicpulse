...
export default {
  async queue(batch: MessageBatch<NewsJobData>, env: Env): Promise<void> {
    console.log(`\n📦 News worker received batch of ${batch.messages.length} jobs`);
    ...
    for (const message of batch.messages) {
      ...
      try {
        ...
        if (!job.forceRefresh) {
          ...
          const cached = await getCachedNews(job.userId, job.interests, job.limit, env);
        ...
        const existingImages = await getTopicImages(job.interests, env);
        console.log(`   ✅ Found ${existingImages.length} existing images`);

        const missingTopics = await getMissingTopicImages(job.interests, env);
        ...
        if (missingTopics.length > 0) {
          ...
          const fetchedImages = await Promise.all(
            missingTopics.map(async (interest: string) => {
              try {
                const image = await getRandomPhoto(interest, env);
                if (image) {
          ...
          if (newTopicImages.length > 0) {
            await saveTopicImages(newTopicImages, env);
        ...
        const savedArticles = await saveNewsArticles(freshArticles, env);
        console.log(`   ✅ Saved ${savedArticles.length} articles`);

        // Step 5: Cache results for 24 hours
        console.log('\n💾 Step 5: Caching results...');
        await storeArticlesInCache(job.userId, savedArticles, 24, env);
