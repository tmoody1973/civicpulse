...
async function getUserProfile(analytics, memory, userId, civicDb) {
  try {
...
async function updateUserProfile(analytics, memory, context) {
  ...
  try {
    ...
    if (!existingProfile) {
      existingProfile = {
        ...
        perplexitySettings: { suggestQuestions: true, saveChatHistory: true, maxTokens: 1e3 },
        learningStyle: "quick",
...
async function storeProfileInMemory(memory, profile, source) {
  try {
...
async function getWidgetPreferences(analytics, userId) {
  try {
...
async function updateWidgetPreferences(analytics, userId, widgetType, updates) {
  try {
    await initializeWidgetPreferencesTable(analytics);
...
async function initializePreferencesTable(analytics) {
  await analytics.executeQuery({
    sqlQuery: `
...
async function initializeWidgetPreferencesTable(analytics) {
  await analytics.executeQuery({
    sqlQuery: `
...
function getDefaultPreferences() {
  return {
    ...
    podcastPreferences: {
      ...
      topics: [],
      // Same as policyInterests
      focus: ["my-reps", "my-state"],
      listeningDays: ["monday", "tuesday", "wednesday", "thursday", "friday"]
    }
  };
}
function getDefaultWidgetPreferences(userId) {
  return [
    { userId, widgetType: "hero", isVisible: true, position: 0, filterSettings: {}, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "legislation", isVisible: true, position: 1, filterSettings: { category: "all" }, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "twitter", isVisible: true, position: 2, filterSettings: { filter: "all" }, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "news", isVisible: true, position: 3, filterSettings: { source: "all" }, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "podcast-queue", isVisible: true, position: 4, filterSettings: {}, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "perplexity-chat", isVisible: true, position: 5, filterSettings: {}, updatedAt: /* @__PURE__ */ new Date() },
    { userId, widgetType: "civic-impact", isVisible: true, position: 6, filterSettings: {}, updatedAt: /* @__PURE__ */ new Date() }
  ];
}
async function migrateUserFromLegacyTable(civicDb, analytics, memory, userId) {
  try {
    ...
    const legacyUser = result;
    console.log("\u{1F4E6} Legacy user data:", {
    ...
    let representatives = [];
    if (legacyUser.state) {
      console.log(`\u{1F50D} Fetching representatives for ${legacyUser.state}, district ${legacyUser.district}`);
    ...
    if (legacyUser.name) {
      ...
      lastName = nameParts.slice(1).join(" ") || void 0;
    }
    const migratedProfile = {
      ...
      lastName,
      preferredLanguage: "en",
      // Default to English for migrated users
      ...
      representatives,
      // Fetched based on users.state/district
      notificationPreferences: {
      ...
      podcastPreferences: {
        ...
        topics: policyInterests,
        // Same as policy interests
        focus: [],
    ...
    console.log("\u{1F4BE} Saving migrated profile to ANALYTICS database...");
...
async function fetchRepresentativesByLocation(civicDb, state, district) {
  try {
    let query = `
      SELECT
        bioguide_id,
    ...
    const result = await civicDb.prepare(query).bind(...params).all();
...
async function saveUserProfile(analytics, profile) {
  await analytics.executeQuery({
    sqlQuery: `
...
// src/preferences/index.ts
var SUPPORTED_LANGUAGE_CODES = ["en", "es", "fr", "de", "pt", "it", "pl", "hi", "ja", "ko", "zh", "ar"];
var updateProfileSchema = external_exports.object({
  ...
  updates: external_exports.object({
    ...
    preferredLanguage: external_exports.enum(SUPPORTED_LANGUAGE_CODES).optional(),
    policyInterests: external_exports.array(external_exports.string()).optional(),
    representatives: external_exports.array(external_exports.object({
      bioguideId: external_exports.string(),
      name: external_exports.string(),
      chamber: external_exports.enum(["house", "senate"]),
      state: external_exports.string(),
      district: external_exports.string().nullish(),
      party: external_exports.string()
    })).optional(),
    location: external_exports.object({
      state: external_exports.string(),
      district: external_exports.string().nullish(),
      city: external_exports.string().nullish(),
      zipCode: external_exports.string().nullish()
    }).optional(),
    notificationPreferences: external_exports.object({
      email: external_exports.boolean(),
      push: external_exports.boolean(),
      sms: external_exports.boolean(),
      billUpdates: external_exports.boolean(),
      representativeActivity: external_exports.boolean(),
      podcastReady: external_exports.boolean(),
      newsAlerts: external_exports.boolean(),
      quietHours: external_exports.object({
        enabled: external_exports.boolean(),
        start: external_exports.string(),
        end: external_exports.string()
      })
    }).optional(),
    newsSources: external_exports.array(external_exports.string()).optional(),
    twitterFeedEnabled: external_exports.boolean().optional(),
    twitterFeedFilters: external_exports.object({
      showAllReps: external_exports.boolean(),
      showOnlyMyReps: external_exports.boolean(),
      showHouseOnly: external_exports.boolean(),
      showSenateOnly: external_exports.boolean(),
      specificReps: external_exports.array(external_exports.string()).optional()
    }).optional(),
    perplexityEnabled: external_exports.boolean().optional(),
    perplexitySettings: external_exports.object({
      suggestQuestions: external_exports.boolean(),
      saveChatHistory: external_exports.boolean(),
      maxTokens: external_exports.number()
    }).optional(),
    podcastPreferences: external_exports.object({
      autoGenerate: external_exports.boolean(),
      generationTime: external_exports.string().optional(),
      preferredLength: external_exports.enum(["quick", "standard", "in-depth"]),
      topics: external_exports.array(external_exports.string()),
      focus: external_exports.array(external_exports.string()),
      listeningDays: external_exports.array(external_exports.string())
    }).optional(),
    learningStyle: external_exports.enum(["quick", "detailed", "audio-focused"]).optional()
  }),
  source: external_exports.string().optional().default("settings"),
  metadata: external_exports.record(external_exports.any()).optional()
});
var updateWidgetSchema = external_exports.object({
  userId: external_exports.string().min(1),
  widgetType: external_exports.enum([
    "hero",
    "legislation",
    "twitter",
    "news",
  ...
  updates: external_exports.object({
    ...
    filterSettings: external_exports.record(external_exports.any()).optional()
  })
});
var PreferencesService = class extends Service {
  async fetch(request) {
    ...
    try {
      ...
      if (url.pathname === "/api/preferences/profile" && request.method === "PATCH") {
        ...
        if (updates.firstName === null) updates.firstName = void 0;
        if (updates.lastName === null) updates.lastName = void 0;
        if (updates.location) {
          if (updates.location.city === null) updates.location.city = void 0;
          if (updates.location.district === null) updates.location.district = void 0;
          if (updates.location.zipCode === null) updates.location.zipCode = void 0;
        }
        if (updates.representatives) {
          updates.representatives = updates.representatives.map((rep) => ({
            ...rep,
            district: rep.district === null ? void 0 : rep.district
          }));
        }
...
// <stdin>
var stdin_default = PreferencesService;
export {
  cors,
