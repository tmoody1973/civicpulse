...
async function trackInteraction(memory, analytics, context, options = {}) {
  const {
    userId,
    interactionType,
    targetId,
...
async function storeInMemory(memory, userId, context, sessionId) {
  let workingMemory;
  if (sessionId) {
...
async function storeInDatabase(analytics, data) {
...
function formatInteractionForMemory(context) {
  const { interactionType, targetId, metadata = {} } = context;
  const parts = [`User ${interactionType.replace("_", " ")}`];
  if (targetId) {
  ...
  if (metadata.completionRate !== void 0) {
...
var trackingSchema = external_exports.object({
  ...
  interactionType: external_exports.enum([
    "bill_view",
    "bill_track",
    "podcast_listen",
    "podcast_complete",
  ...
  targetType: external_exports.enum(["bill", "podcast", "article", "tweet", "representative"]).optional(),
  metadata: external_exports.record(external_exports.any()).optional(),
  sessionId: external_exports.string().optional()
});
var TrackingService = class extends Service {
  async fetch(request) {
    const corsHeaders = {
...
// <stdin>
var stdin_default = TrackingService;
export {
  cors,
